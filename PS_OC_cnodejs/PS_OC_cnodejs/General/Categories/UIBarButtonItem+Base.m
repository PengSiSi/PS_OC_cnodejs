...
+ (UIBarButtonItem *)itemWithImage:(NSString *)image higlightedImage:(NSString *)selectedImage target:(id)target action:(SEL)action
{
    // 1.创建按钮
...
+ (UIBarButtonItem *)itemWithImage:(NSString *)image higlightedImage:(NSString *)higlightedImage title:(NSString *)title target:(id)target action:(SEL)action {
    return [self setItemWithImage:image higlightedImage:higlightedImage title:title titleColor:BLUE_COLOR font:NAV_FONT target:target action:action];
}
